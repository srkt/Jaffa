"use strict";
var Jaffa;
(function (Jaffa) {
    var Observable = (function () {
        function Observable(observe) {
            this.observe = observe;
            this.actionNames = {
                onNext: 'onNext',
                onError: 'onError',
                onComplete: 'onComplete'
            };
            var uid = 0;
            this.observe(this._observableActions);
            for (var actionName in this.actionNames) {
                this._observableActions[actionName] = this[actionName];
            }
            this.getUid = {
                next: function () {
                    return uid++;
                }
            };
        }
        Observable.prototype.subscribe = function (onNext, onError, onComplete) {
            var _this = this;
            var subscribeObject = {
                onNext: onNext,
                onError: onError,
                onComplete: onComplete,
                id: this.getUid.next()
            };
            this._subscribers.push(subscribeObject);
            return {
                dispose: function () {
                    var idx = _this._subscribers.indexOf(subscribeObject);
                    if (idx) {
                        _this._subscribers.splice(idx, 1);
                    }
                }
            };
        };
        Observable.prototype.invoke = function (args, funcName) {
            this._subscribers.forEach(function (subscriber) {
                subscriber[funcName](args);
            });
        };
        Observable.prototype.onNext = function (args) {
            this.invoke(args, this.actionNames.onNext);
        };
        Observable.prototype.onError = function (args) {
            this.invoke(args, this.actionNames.onError);
        };
        Observable.prototype.onCompleted = function (args) {
            this.invoke(args, this.actionNames.onComplete);
        };
        return Observable;
    }());
    Jaffa.Observable = Observable;
    var Element = (function () {
        function Element() {
        }
        return Element;
    }());
    Jaffa.Element = Element;
})(Jaffa = exports.Jaffa || (exports.Jaffa = {}));
//# sourceMappingURL=Jaffa.js.map